...
- [ ] the stale-recheck-window bug (both director-loop threads stuck at `recheck_after_days=10`) also encoded as a regression case — **partially addressed 2026-07-27:** `TestsLocal.js` now covers `OpenLoops.upsert`/`getDue`'s due-date arithmetic (elapsed vs. not-yet-due vs. closed, `ignoreDue`, and `_sanitizeRecheckDays` clamping), so a future code regression there can't silently reproduce the "never bumped" symptom. But re-checked against the actual incident write-up and confirmed this checklist item can't be fully closed by a local test: the real root cause was the model's own judgment (seasonal dead-month restraint overriding an explicit stated blocker, plus a flag-over-nudge default) — already fixed via `Context.js`/`SystemPrompt.js` prompt content, not code, so it isn't reproducible as deterministic pure-logic input/output the way the recipient-completion bug was. Closing this item for real needs the live-data dry-run scenario (checklist item below), which needs `ReadApi`/`WriteApi` credentials — not something `TestsLocal.js` should ever touch.
