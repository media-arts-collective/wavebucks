...
/** Keep sentence boundaries, which `strip` would flatten into spaces. Knowing
 *  where a sentence starts is the whole basis of the name check below. */
const toLines = html => String(html || '')
  .replace(/<\/(p|li|h\d|div|tr)>|<br\s*\/?>/gi, '\n')
  .replace(/<[^>]+>/g, ' ')
  .replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
  .replace(/&nbsp;/g, ' ');
...
const vocabulary = text => new Set(
  (strip(text).match(/\b[a-zA-Z'’-]{2,}\b/g) || []).map(norm)
);
...
const figures = text => new Set(
  (strip(text).match(/(?<![.\d])\$?\d[\d,.:]*(?:ms|am|pm|%)?\b/gi) || [])
    .map(n => n.toLowerCase().replace(/[.,]$/, ''))
    .filter(n => !/^\d\.?$/.test(n))
);

const itemNumbers = text => (strip(text).match(/(?:^|\s)(-?\d+)\.\s/g) || [])
...
export function runChecks(d, notes, vault) {
  ...
  if (!body) add('fail', 'body', 'no body_html');
  ...
  if (!/<3\s*SM\b/.test(strip(body))) {
    add('fail', 'sign-off', 'missing the `<3 SM` sign-off');
  }
  if (/\bMS\b/.test(strip(body).replace(/<3\s*SM/g, ''))) {
  ...
  //    obligations.
  if (vault?.motifs?.['all-caps-emphasis'] && !/\b[A-Z]{4,}\b/.test(strip(body))) {
    add('warn', 'no-caps',
